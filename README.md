...
本项目基于 Streamlit 构建轻量化可视化工具，旨在服务于中国利率债ETF的主动量化做市策略。当前核心研究对象为 **富国中债7-10年政金债ETF（511520.SH）**，并使用 **10年期国债期货（T2509合约）和5年期国债期货** 进行 DV01 中性关键久期对冲（最小二乘向量软约束精细化对冲）计算与建议。
...
结合 LARS 模型对未来价差变化方向的预测，构建 ETF–期货 的方向性对冲策略。通过模型信号控制交易行为，追求在市场趋势出现时捕捉超额收益，并借助对冲机制控制波动。

### 策略逻辑说明

1. **建仓信号**：
   * 若前一日预测信号为正（即未来价差预期上行），则在当前交易日开盘：
     * 用约 90% 的资金买入 ETF；
     * 同时按等值名义资金卖出国债期货进行空头对冲；
2. **平仓信号**：
   * 若前一日预测信号为负或为 0，表示不看多价差，平掉全部持仓；
   
3. **每日滚动检查**持仓状态与交易信号；
   
4. **考虑交易成本与冲击成本**：

   * ETF 开/平仓各劣于市价 3 tick；
   * 期货开/平仓各劣于 1 tick。
...
> 当前信号触发次数偏少，策略处于观望状态多数，收益偏低但回撤控制优秀。

---

### 策略净值曲线示意图
<img width="1189" height="590" alt="image" src="https://github.com/user-attachments/assets/569c6ca0-4622-407b-9027-f75d4e1f46ea" />
...
| 方向       | 说明                              |
| -------- | ------------------------------- |
| 更换预测周期   | 试用 `shift_days=3~5` 的模型信号，准确率更高 |
| 增加信号强度判断 | 设置信号阈值（如预测值绝对值超过某值才触发交易）        |
| 循环优化仓位比例 | 测试ETF头寸比例50\~90%，期货头寸保持等值对冲     |
| 增加交易频率   | 采用滚动窗口、双边布林带触发增强信号密度           |
| 纳入滑点成本模型 | 根据L2级别估计真实成交价，增强实战模拟         |

---

### 回测表现指标矩阵（不同参数组合）

| 信号延迟 (天) | ETF仓位比例 | 年化收益      | 最大回撤  | Sharpe比  | Calmar比   | 胜率    |
| -------- | ------- | --------- | ----- | -------- | --------- | ----- |
| 1        | 0.9     | 0.32%     | 0.07% | 1.48     | 4.71      | 100%  |
| 5        | 0.8     | **2.91%** | 0.23% | **1.57** | **12.65** | 61.5% |
| 4        | 0.9     | 2.65%     | 0.19% | 1.43     | 13.95     | 64.3% |
| ...      | ...     | ...       | ...   | ...      | ...       | ...   |

> 该表为优化网格回测结果，表明最佳策略组合可能出现在 `shift_days=4~5`、`ETF比例=80%~90%` 区间。

---

### 下一步

* [ ] 将预测信号切换为 `5日预测`，调用已有模型；
